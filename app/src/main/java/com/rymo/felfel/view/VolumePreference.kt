...
import com.rymo.felfel.background.PluginAlarmData
import com.rymo.felfel.background.TargetVolume
import com.rymo.felfel.configuration.Prefs
import com.rymo.felfel.configuration.Prefs.Companion.MAX_PREALARM_VOLUME
...
class VolumePreference(mContext: Context, attrs: AttributeSet) : Preference(mContext, attrs) {
    ...
    /** Called from [com.rymo.felfel.features.SettingsFragment.onResume] */
    fun onResume() {
    ...
    /** Called from [com.rymo.felfel.features.SettingsFragment.onPause] */
    fun onPause() {
        stopMasterSample()
        stopPrealarmSample()
    }
    ...
    private fun bindPrealarmSeekBar(preAlarmSeekBar: SeekBar) {
        ...
        preAlarmSeekBarProgress.subscribeIn(fragmentScope) { progress ->
            ...
            prealarmSampleDisposable =
                klaxon.go(
                    PluginAlarmData(id = -1, label = "", alarmtone = Alarmtone.Default, simId = 0, delay = 0),
                    prealarm = true,
